...
3. Ввел индивидуальную ответственность на промисы коммитов - раньше еси истекало время общего таймера все даты коммитов блокировались, ни один элемент списка не отрисовывался. Теперь к каждому запросу даты коммита прицеплен свой таймер, если таймер истекает - только у этого элемента списка будет прочерк вместо даты. А репы отрисуются все
